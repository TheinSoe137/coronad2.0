...
export default function ArtGallery() {
  ...
  return (<>
    ...
    {/* heading */}
      <Navbar/>
    <h1 className="font-bold text-xl mt-30 md:text-6xl text-center bg-clip-text text-transparent bg-gradient-to-b from-neutral-50 to-neutral-400 py-4">
        ...
        >
          {images.map((src, idx) => (
            <motion.div key={idx} variants={item} whileHover={{ scale: 1.05 }}   onClick={() => setCurrent(idx)}>
              <img src={src} alt={`Art ${idx + 1}`}  className={`rounded-lg shadow-lg w-full h-40 lg:h-48 object-cover border-2 ${
                  current === idx ? "border-purple-500" : "border-transparent"
                }`} />
            </motion.div>
          ))}
        </motion.div>
  ...
  >
    {images.map((src, idx) => (
      <motion.div
        ...
        className="min-w-[8rem]" // or w-40, adjust as needed
      >
        <img
          src={src}
          alt={`Art ${idx + 1}`}
          className={`rounded-lg shadow-lg w-full h-40 object-cover border-2 ${
            current === idx ? "border-purple-500" : "border-transparent"
          }`}
        />
      </motion.div>
      ...
      {/* Character Lore */}
      <section className=" max-w:1/2 min-h-screen text-white px-4 py-4 md:py-16 flex">  <AnimatePresence mode="wait">
        <motion.div
          key={current}
          variants={variants}
          initial="enter"
          ...
          className="space-y-6 align-center text-center sticky top-10 bottom-10 w-full max-w-md m-auto "
        >
          <img
            src={characters[current].image}
            alt={characters[current].name}
            className="w-[300px] m-auto h-64 object-cover rounded-xl shadow-md"
          />
